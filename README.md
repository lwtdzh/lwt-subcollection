...
### 3. 配置订阅和管理入口：
  ...
  例如您的pages项目域名为：`sub.fuck.cloudns.biz`；
   - 订阅入口固定为 `/sub`，例如 `https://sub.fuck.cloudns.biz/sub`
   - 管理入口固定为 `/admin`，例如 `https://sub.fuck.cloudns.biz/admin`
   - 添加 `ADMIN_PWD` 变量作为 `/admin` 的管理密码

### 4. 添加你的节点和订阅链接：
   1. 绑定**变量名称**为`KV`的**KV命名空间**；
   2. 访问 `https://sub.fuck.cloudns.biz/admin`，添加你的自建节点链接和机场订阅链接，确保每行一个链接，例如：
      ```
...
### 2. 订阅和管理入口：
  ...
  例如您的workers项目域名为：`sub.cmliussss.workers.dev`；
   - 订阅入口固定为 `/sub`，不同客户端可使用查询参数选择格式。
     ```url
     https://sub.cmliussss.workers.dev/sub
     https://sub.cmliussss.workers.dev/sub?b64
     https://sub.cmliussss.workers.dev/sub?clash
     https://sub.cmliussss.workers.dev/sub?sb
     https://sub.cmliussss.workers.dev/sub?surge
     https://sub.cmliussss.workers.dev/sub?loon
     ```
   - 管理入口固定为 `/admin`，需要配置 `ADMIN_PWD` 变量作为管理密码。


### 3. 添加你的节点或订阅链接：
   1. 绑定**变量名称**为`KV`的**KV命名空间**；
   2. 访问 `https://sub.cmliussss.workers.dev/admin`，添加你的自建节点链接和机场订阅链接，确保每行一个链接，例如：
      ```
      vless://b7a392e2-4ef0-4496-90bc-1c37bb234904@cf.090227.xyz:443?encryption=none&security=tls&sni=edgetunnel-2z2.pages.dev&fp=random&type=ws&host=edgetunnel-2z2.pages.dev&path=%2F%3Fed%3D2048#%E5%8A%A0%E5%85%A5%E6%88%91%E7%9A%84%E9%A2%91%E9%81%93t.me%2FCMLiussss%E8%A7%A3%E9%94%81%E6%9B%B4%E5%A4%9A%E4%BC%98%E9%80%89%E8%8A%82%E7%82%B9
      vmess:
      https://sub.xf.free.hr/sub
